...
export default async function AuditPage() {
  ...
  return <div className="content">
    ...
    <section className="table-card"><div className="table-head"><div><h2>Recent events</h2><p>{events.length} events visible to your role</p></div></div><div className="table-scroll"><table><thead><tr><th>Event</th><th>Actor</th><th>Subject</th><th>Time</th></tr></thead><tbody>{events.map((event) => <tr key={event.id}><td><b>{event.eventType.replaceAll("_", " ")}</b></td><td>{event.actorName}</td><td>{event.claimReference ? <Link href={`/claims/${event.claimReference}`}>{event.claimReference}</Link> : event.subjectName ?? "—"}</td><td>{new Intl.DateTimeFormat("en-MU", { dateStyle: "medium", timeStyle: "short" }).format(new Date(event.createdAt))}</td></tr>)}</tbody></table>{events.length === 0 ? <div className="empty-state"><p>No audit events are available yet.</p></div> : null}</div></section>
  </div>;
}
