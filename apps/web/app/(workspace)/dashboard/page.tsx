...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<{ denied?: string }> }) {
  const [viewer, claims, informationRequired, { denied }] = await Promise.all([
    requireViewer(),
    listClaims("", 6),
    listClaims("", 50, "INFORMATION_REQUIRED"),
    searchParams,
  ]);
  const review = claims.filter((claim) => claim.status === "REVIEW_REQUIRED");
  const processing = claims.filter((claim) => claim.status === "PROCESSING" || claim.status === "UPLOADED" || claim.status === "INFORMATION_RECEIVED");
  ...
  const attention = isClient ? informationRequired[0] ?? review[0] : managementAttention ?? review[0];
  ...
  return <div className="content">
    ...
    <section className="metrics">
      <article><div className="metric-icon amber"><AlertTriangle /></div><span>{isClient ? "Action required" : "Review required"}</span><strong>{isClient ? informationRequired.length : review.length}</strong><small><b>{isClient ? "Your response" : "Action needed"}</b> before verification</small></article>
    ...
    {attention ? <Link className="attention" href={attentionHref}><div className="attention-icon"><Sparkles /></div><div><span>{isClient && attention.status === "INFORMATION_REQUIRED" ? "Your response is required" : isClient ? "Claim update" : attention.status === "VERIFIED" ? "Decision required" : "Ready for human review"}</span><h3>{attention.reference} · {attention.patientName}</h3><p>{isClient && attention.status === "INFORMATION_REQUIRED" ? "A Claims Officer has requested clarification or supporting documents." : isClient ? "Your claim is awaiting an accountable human decision." : attention.status === "VERIFIED" ? "Evidence has been verified and is ready for a supervisor decision." : `${attention.warningCount} validation warning needs attention. Source evidence is ready.`}</p></div><b>{attention.status === "VERIFIED" ? "Decide claim" : isClient && attention.status === "INFORMATION_REQUIRED" ? "Respond now" : isClient ? "View claim" : "Review claim"} <ArrowRight /></b></Link> : null}
