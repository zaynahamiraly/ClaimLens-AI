...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<{ denied?: string }> }) {
  ...
  const attentionHref = attention ? isClient ? `/claims/${attention.reference}` : `/claims/${attention.reference}/review` : "/claims";
  ...
  return <div className="content">
    ...
    {attention ? <Link className="attention" href={attentionHref}><div className="attention-icon"><Sparkles /></div><div><span>{isClient ? "Claim update" : "Ready for human review"}</span><h3>{attention.reference} · {attention.patientName}</h3><p>{isClient ? "Your claim is awaiting an accountable human decision." : `${attention.warningCount} validation warning needs attention. Source evidence is ready.`}</p></div><b>{isClient ? "View claim" : "Review claim"} <ArrowRight /></b></Link> : null}
