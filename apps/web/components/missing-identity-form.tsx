...
export function MissingIdentityForm({
  ...
  return <section className="missing-identity"><div><AlertTriangle /><span><b>Information required</b><small>OCR could not confidently detect these values. Enter them from the source document before verification.</small></span></div><form action={submitAction}>{patientMissing ? <label>Patient name<input name="patientName" minLength={2} maxLength={120} required /></label> : null}{providerMissing ? <label>Provider name<input name="providerName" minLength={2} maxLength={160} required /></label> : null}{amountMissing ? <><label>Claimed amount<input name="claimedAmount" type="number" min="0.01" step="0.01" required /></label><label>Currency<select name="claimedCurrency" defaultValue="" required><option value="" disabled>Select currency</option><option value="MUR">MUR</option><option value="UGX">UGX</option><option value="KES">KES</option><option value="TZS">TZS</option><option value="USD">USD</option><option value="EUR">EUR</option><option value="GBP">GBP</option></select></label></> : null}<button className="secondary" type="submit" disabled={pending}><Save />{pending ? "Saving…" : "Save information"}</button>{state.error ? <small className="workflow-error" role="alert">{state.error}</small> : null}{state.success ? <small className="success-text" role="status">Saved as human-provided information.</small> : null}</form></section>;
}
